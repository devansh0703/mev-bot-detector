...
function loadWasm() {
    try {
        ...
        logger.info("✅ Rust/WASM engine loaded successfully.");
        return engine;
    } catch (err) {
        logger.fatal({ err }, "❌ Failed to load critical WASM module. This is often an issue with the build process or file paths. Exiting.");
        process.exit(1);
    }
}
...
// --- Main Application ---
async function main() {
    logger.info("🚀 Starting MEV-Bot Detector Service in production mode...");
    ...
    logger.info({ batchSize: config.batchSize, interval: config.batchIntervalMs }, "🎧 Listening for pending transactions...");
...
async function processTransactionBatch(txBatch, mevEngine, producer, redis) {
    ...
    try {
        ...
        if (detectedAttacks.length > 0) {
            logger.info({ count: detectedAttacks.length }, `🔥 Detected potential sandwich attack(s)!`);
...
async function handleDetectedAttack(attack, producer, redis) {
    ...
    if (!isNew) {
        logger.info({ attacker: attackerAddress }, `🤫 Duplicate alert suppressed for attacker.`);
        return;
    }
    ...
    logger.warn({ attacker: attackerAddress, victim: attack.victim_tx_hash }, `🚨 New valid MEV attacker found. Sending alert!`);
...
async function checkAttackerHistory(address) {
    ...
    try {
        ...
        logger.error({ err, address }, `❌ Error querying The Graph for address history.`);
...
// --- Entry Point ---
main().catch(err => {
    logger.fatal({ err }, "❌ Unhandled error in main function. Service is stopping.");
    process.exit(1);
});
